...
class EmailService {
   ...
   */
  static async sendEventReminder(event, recipientEmail) {
    try {
      if (!recipientEmail) {
   ...
   * @returns {Promise<Object>} Resultado da operação.
   */
  static async syncEventsViaEmail(events, recipientEmail) {
    try {
      if (!recipientEmail) {
        throw new Error('Email do destinatário não fornecido');
      }
      ...
      const eventsHTML = events.map(event => `
        <div style="margin-bottom: 20px; padding: 15px; border: 1px solid #e0e0e0; border-radius: 5px;">
          ...
          ${event.location ? `<p><strong>Local:</strong> ${event.location}</p>` : ''}
          ${event.description ? `<p><strong>Descrição:</strong> ${event.description}</p>` : ''}
          ${event.type ? `<p><strong>Tipo:</strong> ${event.type.charAt(0).toUpperCase() + event.type.slice(1)}</p>` : ''}
        </div>
      `).join('');
   ...
   */
  static async configureEmailAlert(event, recipientEmail, minutesBefore) {
    try {
      if (!event || !event.id) {
